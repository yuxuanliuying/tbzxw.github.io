...
let _close = false;
let _fsj = false;
var url = 'https://yuxuanliuying.github.io/tbzxw.github.io/static/image/0127.png';
...
function showGameScoreLayer() {
    ...
    let score_text = '您坚持了 ';
    score_text += "<span style='color:red;'>" + (deviation_time / 1000).toFixed(2) + "</span>" + ' 秒<br>您的得分为 ';
    ...
    score_text += "</span>" + ' 次';
    score_text += "<br>相当于 <span style='color:red;'>" + (_gameScore * 15000 / deviation_time).toFixed(2) + "</span> BPM 下的十六分音符"
...
function shareText(score) {
    ...
    if (score <= 2.5 * __Time) return '你搞积的';
    if (score <= 5 * __Time) return '均值不等式';
    if (score <= 7.5 * __Time) return '想他了';
    if (score <= 10 * __Time) return '想他想的睡不着';
    return '你怎么死了';
}
